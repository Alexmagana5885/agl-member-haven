...
def get_outstanding_amount(email):
    """
    Calculate the outstanding amount for premium payments in the last 1 year.
    
    Args:
        email: Member's email address
    
    Returns:
        int: Outstanding amount to be paid
    """
    try:
...
def pay_premium():
    ...
    Expected JSON payload:
    {
        ...
        "phone": "0722000000",
        "amount": "3600" (optional, will calculate outstanding if not provided)
    }
    """
    try:
        ...
        if result.get("success"):
            ...
            try:
                ...
                logger.error(f"Database error saving transaction: {str(db_err)}")
                # Continue anyway - the transaction ID was received from M-Pesa
...
def get_outstanding():
    """
    Get the outstanding premium amount for a member.
    
    Query parameters:
        email: Member's email address
    
    Returns:
        JSON response with outstanding amount
    """
    try:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "membership_type": "personal" if base_amount == PERSONAL_PREMIUM_AMOUNT else "organization"
            }
        }), 200
        
    except Exception as e:
        logger.error(f"Error getting outstanding amount: {str(e)}")
        return jsonify({
            "success": False,
...
@premiums_bp.route('/transaction-status/<checkout_request_id>', methods=['GET'])
